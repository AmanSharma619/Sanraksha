...
const About = () => {
  ...
  const handleScroll = () => {
    const elements = document.querySelectorAll(".features-list");
    elements.forEach((el) => {
  ...
  useEffect(() => {
      ...
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          if (entry.target.id === 'first' || 
              entry.target.id === 'second' || 
              entry.target.id === 'third' || 
              entry.target.id === 'fourth' || 
              entry.target.id === 'fifth') {
            entry.target.querySelector('img').classList.add('animate-image');
            entry.target.querySelector('.content').classList.add('animate-content');
          } else if (entry.target.classList.contains('heading')) {
            Array.from(heading.current.children).forEach((child, index) => {
              setTimeout(() => {
                child.classList.add('animate-heading');
              }, index * 100);
            });
          }
        }
    ...
    document.querySelectorAll('.features-list').forEach(section => {
